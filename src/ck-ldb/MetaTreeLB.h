...
#include "BaseLB.h"
#include "MetaTreeLB.decl.h"
#include "json.hpp"
#include <vector>
using json = nlohmann::json;

#define DEBUG__META_TREE_LB_L1 0
#define DEBUG__META_TREE_LB_L2 0
#define DEBUG__META_TREE_LB_L3 0

void CreateMetaTreeLB();
...
public:
    ...
    virtual MetaTreeLBMessage* getStats() { CkAbort("LevelLogic::getStats not implemented\n"); }
     ...
     */
    virtual MetaTreeLBMessage* mergeStats()
    {
     ...
     */
    virtual MetaTreeLBMessage*  loadBalance(IDM& idm)
    {
     ...
     */
    virtual std::vector<MetaTreeLBMessage*> splitDecision(MetaTreeLBMessage* decision,
     ...
     */
    virtual void processDecision(MetaTreeLBMessage* decision, int& incoming, int& outgoing)
    {
    ...
    virtual int getTokenSets(MetaTreeLBMessage* transferMsg,
    ...
    virtual int tokensReceived(MetaTreeLBMessage* token_set)
    {
...
protected:
    std::vector<MetaTreeLBMessage*> stats_msgs;
};
...
class MetaTreeLB : public CBase_MetaTreeLB
{
public:
    MetaTreeLB(const CkLBOptions& opts) : CBase_MetaTreeLB(opts)
    {
      loadConfigFile(opts);
      init(opts);
    }
    MetaTreeLB(CkMigrateMessage* m) : CBase_MetaTreeLB(m) {}
    virtual ~MetaTreeLB();
...
private:
    ...
    void receiveStats(MetaTreeLBMessage* stats, int level);
    ...
    void receiveDecision(MetaTreeLBMessage* decision, int level);

    void transferLoadToken(MetaTreeLBMessage* transferMsg, int level);
