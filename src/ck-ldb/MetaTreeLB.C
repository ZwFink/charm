...
static void lbinit()
{
  ...
  LBRegisterBalancer<MetaTreeLB>(
          "MetaTreeLB", "Pluggable hierarchical LB with available strategies:" + o.str());
}

void MetaTreeLB::Migrated(int waitBarrier)
{
  objMovedIn(waitBarrier);
}

void MetaTreeLB::loadConfigFile(const CkLBOptions& opts)
{
  config.clear();
  ...
  if (opts.getLegacyName() != nullptr)
  {
    ...
    if (CkMyPe() == 0 && !quietModeRequested)
      CkPrintf("[%d] MetaTreeLB in LEGACY MODE support\n", CkMyPe());
  }
  else if (ifs.good())
  {
    ...
    {
      CkPrintf(
              "Error reading MetaTreeLB configuration file: %s.\n"
  ...
  else
  {
    if (CkMyPe() == 0 && !quietModeRequested)
      CkPrintf(
              "[%d] No MetaTreeLB configuration file found. Choosing a default configuration.\n",
              CkMyPe());
...
void MetaTreeLB::init(const CkLBOptions& opts)
{
#if CMK_LBDB_ON
  ...
  startLbFnHdl = lbmgr->AddStartLBFn(this, &MetaTreeLB::StartLB);

  configure(config);

  // TODO this functionality needs to move to LBManager -> could also move MetaLB data collection here?
  if (_lb_args.statsOn())
...
void MetaTreeLB::configure(LBTreeBuilder& builder, json& config)
{
#if CMK_LBDB_ON

  if (numLevels > 0 && CkMyPe() == 0 && !quietModeRequested)
  {
    CkPrintf("[%d] Reconfiguring MetaTreeLB\n", CkMyPe());
  }
...
void MetaTreeLB::configure(json& config)
{
#if CMK_LBDB_ON
  ...
  {
    CkAbort("MetaTreeLB: configured tree not recognized\n");
  }
#endif
}

void MetaTreeLB::pup(PUP::er& p)
{
...
void MetaTreeLB::InvokeLB()
{
#if CMK_LBDB_ON
  // NOTE: I'm assuming new LBManager will know when (and when not to) call AtSync
  if (barrier_before_lb)
  {
    contribute(CkCallback(CkReductionTarget(MetaTreeLB, ProcessAtSync), thisProxy));
  }
  else
  {
...
void MetaTreeLB::ProcessAtSync()
{
  ...
  MetaTreeLBMessage* stats = logic[level]->getStats();
  stats->level = level;
...
void MetaTreeLB::sendStatsUp(CkMessage* msg)
{
  MetaTreeLBMessage* stats = (MetaTreeLBMessage*)msg;
  ...
  else
  {
    ...
    {
      MetaTreeLBMessage* newMsg = logic->mergeStats();
      newMsg->level = level;
...
void MetaTreeLB::receiveStats(MetaTreeLBMessage* stats, int level)
{
  level += 1;
  awaitingLB[level] = true;
  ...
  if (l->numStatsReceived() == expected_msgs)
  {
    ...
    {
      MetaTreeLBMessage* newMsg = l->mergeStats();
      newMsg->level = level;
...
void MetaTreeLB::loadBalanceSubtree(int level)
{
  if (!awaitingLB[level]) return;
  awaitingLB[level] = false;
  if (level == 0) return lb_done();

  // CkPrintf("[%d] MetaTreeLB::loadBalanceSubtree - level=%d\n", CkMyPe(), level);

  /// CkMessage *inter_subtree_migrations = nullptr;
  IDM idm;
  MetaTreeLBMessage* decision = logic[level]->loadBalance(idm);
  if (idm.size() > 0)
  {
...
void MetaTreeLB::multicastIDM(const IDM& mig_order, int num_pes, int* _pes)
{
...
void MetaTreeLB::sendDecisionDown(CkMessage* msg)
{
  MetaTreeLBMessage* decision = (MetaTreeLBMessage*)msg;
  ...
  else
  {
    ...
    std::vector<MetaTreeLBMessage*> decisions =
...
void MetaTreeLB::receiveDecision(MetaTreeLBMessage* decision, int level)
{
  // fprintf(stderr, "[%d] MetaTreeLB::receiveDecision, level=%d\n", CkMyPe(), level);
  ...
  // fprintf(stderr, "[%d] MetaTreeLB::receiveDecision, deleting decision msg\n", CkMyPe());
...
void MetaTreeLB::transferLoadToken(MetaTreeLBMessage* transferMsg, int level)
{
  ...
  if (logic[level]->makesTokens())
  {
    ...
    std::vector<MetaTreeLBMessage*> token_sets;
  ...
  else
  {
    ...
    CkAbort("MetaTreeLB::transferLoadToken - NOT IMPLEMENTED\n");
  }
}

void MetaTreeLB::recvLoadTokens(CkMessage* tokens)
{
  MetaTreeLBMessage* token_set = (MetaTreeLBMessage*)tokens;
...
void MetaTreeLB::objMovedIn(bool waitBarrier)
{
...
void MetaTreeLB::lb_done()
{
  ...
  // again, MetaTreeLB shouldn't be doing these things. it should just notify LBManager
...
void MetaTreeLB::reportLbTime(double* times, int n)
{
